...
def oscillating_variance(max_iter, curr_iter):
        k=1
        T=(2*max_iter)/(3+2*k)
        return (max_zeta+min_zeta)/2+((max_zeta-min_zeta)/2)*np.cos((2*np.pi*curr_iter)/T)

class CustomCallback(keras.callbacks.Callback):
    def on_epoch_begin(self, epoch, logs):
        zeta = oscillating_variance(maxepoches,epoch)
        print("Zeta=",zeta)
...
max_zeta = 0.3 #the maximum fraction of the weights removed
min_zeta = 0.01 #the minimum fraction of the weights removed
